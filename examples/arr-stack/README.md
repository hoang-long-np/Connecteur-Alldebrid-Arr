...
Tous les conteneurs montent le même dossier de l'hôte (`DATA_PATH`) sur `/data` :

```
/data
├── torrents/          téléchargements du connecteur, un sous-dossier par catégorie
│   ├── radarr/
│   └── tv-sonarr/
└── media/
...
Les chemins sont les mêmes dans tous les conteneurs : aucun *Remote Path Mapping* n'est nécessaire. Comme téléchargements et bibliothèques sont sur le même volume, l'import est un simple déplacement, instantané.
...
2. **Créer les dossiers** sur l'hôte, par exemple :
   ```bash
   sudo mkdir -p /mnt/tank/data/torrents /mnt/tank/data/media/movies /mnt/tank/data/media/tv
   ```
...
Radarr et Sonarr savent exporter leur configuration (*System → Backup*) et la restaurer dans la nouvelle instance. Après restauration, mettre à jour les dossiers racine (`/data/media/...`) et le client de téléchargement (`alldebrid-arr`, port `8090`), qui pointent encore vers les anciens chemins.
